...
sc1.prototype.init = function() {
	...
	this.camera = new THREE.PerspectiveCamera( 25, 700/700, 1, 10000 );
	...
	this.container = document.getElementById( 'moohaha' );
...
sc1.prototype.addGeo = function(){
	...
	var geometry2 = new THREE.SphereGeometry3( 1, 7, 7,3,2,.3);
	...
	for ( var i = 0 ; i < 700 ; i++){
		...
		var rot = new THREE.Vector3(randly(-.5,1),randly(-.5,1),randly(-.5,1));
		var sc = randly(1,5);
		var ran = new THREE.Vector3(sc,sc/3,sc*2);
		mesh.position = randy;
		mesh.scale = ran;
		mesh.rotation = rot;
		this.scene.add(mesh);
...
sc1.prototype.moveThings = function(){
	...
	for ( var j = 0 ; j <   this.things.length  ; j++){
		...
		var offs = map_range(Math.sin(this.offer),0,1,0,.01);
		...
		for (var i = 1 ; i< thing.rt.length; i++){
			...
			thing.rt[i].rotation.x = (Math.sin((i/this.text.size)+num)*(i*this.text.x+offs))+(Math.sin(this.text.x2)*(i/thing.rt.length))+(Math.sin(this.text.x1));
		...
		num-=i*this.text.speed*this.text.speed2+offs;
	}	
